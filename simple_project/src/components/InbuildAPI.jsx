import { useState } from 'react'

export default function InbuildAPI() {


    const ClothData = [
        {
            img: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=500&q=80',
            title: 'Classic White T-Shirt',
            price: 799,
            discount: '20%',
            brand: 'Roadster'
        },
        {
            img: 'https://images.unsplash.com/photo-1503342217505-b0a15ec3261c?auto=format&fit=crop&w=500&q=80',
            title: 'Casual Denim Jacket',
            price: 1999,
            discount: '15%',
            brand: 'Levis'
        },
        {
            img: 'https://images.unsplash.com/photo-1596755389378-c31d21fd1273?auto=format&fit=crop&w=500&q=80',
            title: 'Blue Casual Shirt',
            price: 1199,
            discount: '25%',
            brand: 'Allen Solly'
        },
        {
            img: 'https://images.unsplash.com/photo-1618354691373-d851c5c3a990?auto=format&fit=crop&w=500&q=80',
            title: 'Black Oversized T-Shirt',
            price: 899,
            discount: '30%',
            brand: 'H&M'
        },
        {
            img: 'https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?auto=format&fit=crop&w=500&q=80',
            title: 'Blue Denim Jeans',
            price: 1799,
            discount: '20%',
            brand: 'Levis'
        },
        {
            img: 'https://images.unsplash.com/photo-1594633312681-425c7b97ccd1?auto=format&fit=crop&w=500&q=80',
            title: 'Women Casual Top',
            price: 999,
            discount: '35%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1583743814966-8936f37f4678?auto=format&fit=crop&w=500&q=80',
            title: 'Black Cotton T-Shirt',
            price: 699,
            discount: '20%',
            brand: 'Puma'
        },
        {
            img: 'https://images.unsplash.com/photo-1627225924765-552d49cf47ad?auto=format&fit=crop&w=500&q=80',
            title: 'Men Formal Shirt',
            price: 1499,
            discount: '25%',
            brand: 'Peter England'
        },
        {
            img: 'https://images.unsplash.com/photo-1542272604-787c3835535d?auto=format&fit=crop&w=500&q=80',
            title: 'Slim Fit Jeans',
            price: 1899,
            discount: '30%',
            brand: 'Wrangler'
        },
        {
            img: 'https://images.unsplash.com/photo-1564257577054-0f3e7f7e6e95?auto=format&fit=crop&w=500&q=80',
            title: 'Women Summer Dress',
            price: 1599,
            discount: '40%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1551028719-00167b16eac5?auto=format&fit=crop&w=500&q=80',
            title: 'Leather Jacket',
            price: 3499,
            discount: '20%',
            brand: 'Roadster'
        },
        {
            img: 'https://images.unsplash.com/photo-1603252110481-7ba873bf42ab?auto=format&fit=crop&w=500&q=80',
            title: 'White Casual Shirt',
            price: 1299,
            discount: '25%',
            brand: 'Van Heusen'
        },
        {
            img: 'https://images.unsplash.com/photo-1551484650-2c7b8f4e1d7b?auto=format&fit=crop&w=500&q=80',
            title: 'Denim Shorts',
            price: 999,
            discount: '30%',
            brand: 'Levis'
        },
        {
            img: 'https://images.unsplash.com/photo-1576566588028-4147f3842f27?auto=format&fit=crop&w=500&q=80',
            title: 'Grey Hoodie',
            price: 1699,
            discount: '35%',
            brand: 'Nike'
        },
        {
            img: 'https://images.unsplash.com/photo-1556821840-3a63f95609a7?auto=format&fit=crop&w=500&q=80',
            title: 'Black Pullover Hoodie',
            price: 1899,
            discount: '25%',
            brand: 'Adidas'
        },
        {
            img: 'https://images.unsplash.com/photo-1571945153237-4929e783af4a?auto=format&fit=crop&w=500&q=80',
            title: 'Red Polo T-Shirt',
            price: 999,
            discount: '20%',
            brand: 'U.S. Polo'
        },
        {
            img: 'https://images.unsplash.com/photo-1554568218-0f1715e72254?auto=format&fit=crop&w=500&q=80',
            title: 'Green Polo Shirt',
            price: 1099,
            discount: '30%',
            brand: 'Puma'
        },
        {
            img: 'https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=500&q=80',
            title: 'Women Casual Outfit',
            price: 1799,
            discount: '25%',
            brand: 'H&M'
        },
        {
            img: 'https://images.unsplash.com/photo-1485968579580-b6d095142e6e?auto=format&fit=crop&w=500&q=80',
            title: 'Women Denim Outfit',
            price: 2199,
            discount: '35%',
            brand: 'Levis'
        },
        {
            img: 'https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&w=500&q=80',
            title: 'Women Fashion Dress',
            price: 2499,
            discount: '40%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1496747611176-843222e1e57c?auto=format&fit=crop&w=500&q=80',
            title: 'Floral Summer Dress',
            price: 1999,
            discount: '30%',
            brand: 'Biba'
        },
        {
            img: 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?auto=format&fit=crop&w=500&q=80',
            title: 'Women Casual Jeans',
            price: 1699,
            discount: '25%',
            brand: 'Levis'
        },
        {
            img: 'https://images.unsplash.com/photo-1485230895905-ec40ba36b9bc?auto=format&fit=crop&w=500&q=80',
            title: 'Women White Top',
            price: 899,
            discount: '20%',
            brand: 'H&M'
        },
        {
            img: 'https://images.unsplash.com/photo-1566206091558-7f218b696731?auto=format&fit=crop&w=500&q=80',
            title: 'Women Black Dress',
            price: 2299,
            discount: '35%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1544441893-675973e31985?auto=format&fit=crop&w=500&q=80',
            title: 'Casual Denim Shirt',
            price: 1399,
            discount: '25%',
            brand: 'Wrangler'
        },
        {
            img: 'https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?auto=format&fit=crop&w=500&q=80',
            title: 'Men Checked Shirt',
            price: 1299,
            discount: '30%',
            brand: 'Peter England'
        },
        {
            img: 'https://images.unsplash.com/photo-1610652492500-ded49ceeb378?auto=format&fit=crop&w=500&q=80',
            title: 'Men Black Shirt',
            price: 1399,
            discount: '20%',
            brand: 'Van Heusen'
        },
        {
            img: 'https://images.unsplash.com/photo-1564859228273-274232fdb516?auto=format&fit=crop&w=500&q=80',
            title: 'Men Casual Trousers',
            price: 1599,
            discount: '25%',
            brand: 'Allen Solly'
        },
        {
            img: 'https://images.unsplash.com/photo-1516826957135-700dedea698c?auto=format&fit=crop&w=500&q=80',
            title: 'Men Jogger Pants',
            price: 1199,
            discount: '35%',
            brand: 'Adidas'
        },
        {
            img: 'https://images.unsplash.com/photo-1552902865-b72c031ac5ea?auto=format&fit=crop&w=500&q=80',
            title: 'Sports Track Pants',
            price: 1099,
            discount: '30%',
            brand: 'Nike'
        },
        {
            img: 'https://images.unsplash.com/photo-1556905055-8f358a7a47b2?auto=format&fit=crop&w=500&q=80',
            title: 'Sports T-Shirt',
            price: 899,
            discount: '25%',
            brand: 'Adidas'
        },
        {
            img: 'https://images.unsplash.com/photo-1551489186-cf8726f514f8?auto=format&fit=crop&w=500&q=80',
            title: 'Casual Blue Jeans',
            price: 1799,
            discount: '20%',
            brand: 'Wrangler'
        },
        {
            img: 'https://images.unsplash.com/photo-1548883354-7622d03aca27?auto=format&fit=crop&w=500&q=80',
            title: 'Men Winter Jacket',
            price: 2999,
            discount: '30%',
            brand: 'Woodland'
        },
        {
            img: 'https://images.unsplash.com/photo-1544022613-e87ca75a784a?auto=format&fit=crop&w=500&q=80',
            title: 'Women Winter Coat',
            price: 3299,
            discount: '35%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1541101767792-f9b2b1c4f127?auto=format&fit=crop&w=500&q=80',
            title: 'Women Knit Sweater',
            price: 1599,
            discount: '25%',
            brand: 'H&M'
        },
        {
            img: 'https://images.unsplash.com/photo-1608234807905-4466023792f5?auto=format&fit=crop&w=500&q=80',
            title: 'Casual Sweatshirt',
            price: 1399,
            discount: '30%',
            brand: 'Puma'
        },
        {
            img: 'https://images.unsplash.com/photo-1578681994506-b8f463449011?auto=format&fit=crop&w=500&q=80',
            title: 'Men Cotton Kurta',
            price: 1299,
            discount: '20%',
            brand: 'Manyavar'
        },
        {
            img: 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&w=500&q=80',
            title: 'Women Traditional Kurta',
            price: 1499,
            discount: '30%',
            brand: 'Biba'
        },
        {
            img: 'https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?auto=format&fit=crop&w=500&q=80',
            title: 'Women Ethnic Suit',
            price: 2199,
            discount: '35%',
            brand: 'W'
        },
        {
            img: 'https://images.unsplash.com/photo-1612902456551-333ac5afa26e?auto=format&fit=crop&w=500&q=80',
            title: 'Cotton Kurta Set',
            price: 1899,
            discount: '25%',
            brand: 'Fabindia'
        },
        {
            img: 'https://images.unsplash.com/photo-1585488433563-8f6e4b7e0a0b?auto=format&fit=crop&w=500&q=80',
            title: 'Women Casual Shirt',
            price: 1199,
            discount: '30%',
            brand: 'Mango'
        },
        {
            img: 'https://images.unsplash.com/photo-1591369822096-ffd140ec948f?auto=format&fit=crop&w=500&q=80',
            title: 'Women Blazer',
            price: 2499,
            discount: '25%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1617127365659-c47fa864d8bc?auto=format&fit=crop&w=500&q=80',
            title: 'Men Formal Blazer',
            price: 3499,
            discount: '30%',
            brand: 'Raymond'
        },
        {
            img: 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?auto=format&fit=crop&w=500&q=80',
            title: 'Men Formal Trousers',
            price: 1699,
            discount: '20%',
            brand: 'Louis Philippe'
        },
        {
            img: 'https://images.unsplash.com/photo-1607345366928-199ea26cfe3e?auto=format&fit=crop&w=500&q=80',
            title: 'Men Linen Shirt',
            price: 1599,
            discount: '25%',
            brand: 'Raymond'
        },
        {
            img: 'https://images.unsplash.com/photo-1525507119028-ed4c629a60a3?auto=format&fit=crop&w=500&q=80',
            title: 'Fashion Casual Collection',
            price: 1999,
            discount: '35%',
            brand: 'Mango'
        },
        {
            img: 'https://images.unsplash.com/photo-1445205170230-053b83016050?auto=format&fit=crop&w=500&q=80',
            title: 'Premium Fashion Shirt',
            price: 1799,
            discount: '30%',
            brand: 'Zara'
        },
        {
            img: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?auto=format&fit=crop&w=500&q=80',
            title: 'Casual Cotton Pants',
            price: 1299,
            discount: '25%',
            brand: 'Roadster'
        },
        {
            img: 'https://images.unsplash.com/photo-1551487600-7c5d0c0c8f7d?auto=format&fit=crop&w=500&q=80',
            title: 'Men Casual Jacket',
            price: 2299,
            discount: '30%',
            brand: 'Roadster'
        },
        {
            img: 'https://images.unsplash.com/photo-1541099649105-f69ad21f3246?auto=format&fit=crop&w=500&q=80',
            title: 'Women Skinny Jeans',
            price: 1799,
            discount: '25%',
            brand: 'Levis'
        },
        {
            img: 'https://images.unsplash.com/photo-1506629905607-d9c297d9b7d3?auto=format&fit=crop&w=500&q=80',
            title: 'Women Casual Trousers',
            price: 1399,
            discount: '30%',
            brand: 'H&M'
        },
        {
            img: 'https://images.unsplash.com/photo-1509631179647-0177331693ae?auto=format&fit=crop&w=500&q=80',
            title: 'Women Fashion Top',
            price: 999,
            discount: '35%',
            brand: 'Mango'
        },
        {
            img: 'https://images.unsplash.com/photo-1496217590455-aa63a8350eea?auto=format&fit=crop&w=500&q=80',
            title: 'Men Casual Outfit',
            price: 1899,
            discount: '25%',
            brand: 'Jack & Jones'
        },
        {
            img: 'https://images.unsplash.com/photo-1516762689617-e1cffcef479d?auto=format&fit=crop&w=500&q=80',
            title: 'Women Denim Jacket',
            price: 2199,
            discount: '30%',
            brand: 'Levis'
        }
    ];

    const [data, setData] = useState(ClothData)
    let unique = [... new Set(ClothData.map((v) => v.brand))]


    return (

        <div>

            <div className='flex py-10 justify-between px-20'>
                <h1>ALl Product</h1>

                <select name="" id="">
                    {unique.map((v) => (
                        <option value="">{v}</option>
                    ))}
                </select>
            </div>

            <div className='flex flex-wrap'>

                {
                    data.map((v, i) => (
                        <div >
                            <img className='h-150 w-100' src={v.img} alt="" />
                            <h1 >{v.title}</h1>
                            <h1>{v.price}</h1>
                            <h1>{v.discount}</h1>
                            <p>{v.brand}</p>
                        </div>
                    ))
                }


            </div>
        </div>
    )
}
